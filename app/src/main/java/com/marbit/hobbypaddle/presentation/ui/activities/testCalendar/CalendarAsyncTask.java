...
abstract class CalendarAsyncTask extends AsyncTask<Void, Void, Boolean> {
  ...
  final CalendarTestActivity activity;
  final CalendarModel model;
  final com.google.api.services.calendar.Calendar client;
  private final View progressBar;

  CalendarAsyncTask(CalendarTestActivity activity) {
  ...
  protected final Boolean doInBackground(Void... ignored) {
    try {
      ...
      activity.startActivityForResult(
          userRecoverableException.getIntent(), CalendarTestActivity.REQUEST_AUTHORIZATION);
    } catch (IOException e) {
      Utils.logAndShow(activity, CalendarTestActivity.TAG, e);
    }
    return false;
  }
