...
public class AsyncBatchInsertCalendars extends CalendarAsyncTask {
  ...
  public AsyncBatchInsertCalendars(CalendarTestActivity calendarSample, List<Calendar> calendars) {
      ...
      client.calendars().insert(calendar).setFields(CalendarInfo.FIELDS)
          .queue(batch, new JsonBatchCallback<Calendar>() {
            ...
            public void onFailure(GoogleJsonError err, HttpHeaders headers) throws IOException {
              Utils.logAndShowError(activity, CalendarTestActivity.TAG, err.getMessage());
            }
          });
    }
    batch.execute();
